...
<div class="big-container" style="background-color: #f5d7b5; min-height: 100vh; padding: 50px 100px 50px 100px;">
    <div class="container-fluid" style="background-color: white; min-height: 100vh; padding:50px 70px 50px 70px;">
        <h1 class="title" style="color: brown;">Edit Product</h1>
        <form action="/product/{{$product->id}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control{{ $errors->has('stock') ? ' border-danger' : '' }}" id="stock" name="stock" value="{{ old('stock') ?? $product->stock }}">
                <small class="form-text text-danger">{!! $errors->first('stock') !!}</small>
            </div>

            <div class="form-group">
                <label for="price">Price</label>
                <input type="text" class="form-control{{ $errors->has('price') ? ' border-danger' : '' }}" id="price" name="price" value="{{ old('price') ?? $product->price}}">
                <small class="form-text text-danger">{!! $errors->first('price') !!}</small>
            </div>

            <div class="form-group">
                    <label for="description">Description</label>
                    <textarea class="form-control{{ $errors->has('description') ? ' border-danger' : '' }}" id="description" name="description" rows="3">{{old('description') ?? $product->description}}</textarea>
